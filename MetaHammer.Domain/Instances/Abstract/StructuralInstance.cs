...
/// <summary>
/// Abstract base class for instances of structural types.
/// It has access to methods and can hold properties.
/// </summary>
public abstract class StructuralInstance(Guid guid, MetaType metaType) : MetaInstance(guid, metaType)
{
    private List<IProperty> Properties { get; } = new();

    public IReadOnlyCollection<Method> Methods => ((StructuralType)this.Type).Methods;
    ...
    {
        var definition = ValidatePrimitiveType(propertyName, PrimitiveType.MetaGuid);
        ValidateCanAddProperty(propertyName, definition);
